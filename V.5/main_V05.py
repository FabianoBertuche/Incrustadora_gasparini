...
def abrir_nova_tela():
    ...
    janelaDB.title('Registros existentes no Banco de Dados')
...
botaoDB = ttk.Button(framedb, text="Ver Registos Salvos", command= abrir_nova_tela)
botaoDB.grid(row=2, column=3, sticky='we')

botaofecha = ttk.Button(framedb, text="Parar Log", command= fechaJanela)
botaofecha.grid(row=3, column=3, sticky='we')
...
def consultaPlcGravaBanco(PLC, posicaoBanco):
    ...
    while tagStart:
        ...
        if posicao_anterior[0] + 1 < 10 : #voltar para 700
