...
class maquina:
    ...
    def escreveLinhaBatelada(self, address):
        resposta = False
        try:
    ...
    def escreveLinhaGeral(self, address):
        resposta = False
        try:
            # Conectar-se ao dispositivo PLC
            self.master.execute(1, cst.WRITE_SINGLE_REGISTER, 36004, output_value=address)
            #print("status da escrita da linha: ")
            resposta = True
        except:           
            print('erro fatal na escrita da linha')
    ...
    def carregaLinhaGeral(self):
        estado = False
        try:
            # Escrever o valor True na bobina
            self.master.execute(1, cst.WRITE_SINGLE_COIL, 42002, output_value=True)
            #self.master.execute(1, cst.WRITE_SINGLE_COIL, 42000, output_value=False)
            estado = True
            #print("status da escrita da linha: ")
        except:  
