...
class BancoDados():
    ...
    def criaTabelas(self):
        ...
        consultaOpcoes.execute('CREATE TABLE IF NOT EXISTS ctrlSincronismo( \
            ...
            ULTIMA_LINHA_LIDA INTEGER DEFAULT 4, \
            ULTIMO_ID_LIDO INTEGER DEFAULT 1, \
        ...
        if result[0] == 0:
            ...
            consultaOpcoes.execute('INSERT INTO ctrlSincronismo (ULTIMA_LINHA_LIDA, ULTIMO_ID_LIDO) VALUES (4, 0)')
        
        # Efetivar a inserção e fechar conexão
        cnnOpcoes.commit()
        cnnOpcoes.close()
    ...
    def buscaUltimaSinc(self):
        # Estabelecer conexão com o banco de dados
        cnnOpcoes = sqlite3.connect('dbSupervisorio.db')
        consultaOpcoes = cnnOpcoes.cursor()

        # Consulta para obter os valores das colunas da primeira linha
        consultaOpcoes.execute('SELECT ULTIMA_LINHA_LIDA, ULTIMO_ID_LIDO FROM ctrlSincronismo ORDER BY ID LIMIT 1')
    ...
    def salvaUltimaSinc(self, dados):
        cnnConexao = sqlite3.connect('dbSupervisorio.db')
        consultaConexao = cnnConexao.cursor()
        consultaConexao.execute('UPDATE ctrlSincronismo SET ULTIMA_LINHA_LIDA = ?, ULTIMO_ID_LIDO = ? WHERE ID = 1', (dados[0], dados[1]))
