...
def abrir_nova_tela():
    ...
    frameDB1 = LabelFrame(janelaDB, text="Comandos", padx=5, pady=5 )
    frameDB1.grid(row=0, column=0, sticky='we')

    label_nome = tk.Label(frameDB1, text = "Nome")
    label_nome.grid(row = 0, column = 0 )
    botao_voltar = tk.Button(frameDB1, text = 'Fechar', command = frameDB1.destroy)
    botao_voltar.grid(row = 1, column = 0)

    frameDB2 = LabelFrame(janelaDB, text="Dados:", padx=5, pady=5 )
    ...
    # inserir scroll horizontal
    #hsb = ttk.Scrollbar(frameDB2, orient="horizontal", command=treeview.xview)
    #hsb.grid(row=1, column=0, sticky='we', padx=5, pady=5)
    #treeview.configure(xscrollcommand=hsb.set)

    treeview.grid(row=0, column=0, sticky='nsew')

    vsb = ttk.Scrollbar(janelaDB, orient="vertical", command=treeview.yview)
    vsb.grid(row=2, column=0, sticky='e', padx=5, pady=5)
    treeview.configure(yscrollcommand=vsb.set)

    hsb = ttk.Scrollbar(janelaDB, orient="horizontal", command=treeview.xview)
    hsb.grid(row=3, column=0, sticky='we', padx=5, pady=5)
    treeview.configure(xscrollcommand=hsb.set)

    janelaDB.grid_rowconfigure(2, weight=1)
    janelaDB.grid_columnconfigure(0, weight=1)
    ...
    # Função para centralizar os dados nas colunas do Treeview
    def center_data(event):
        treeview.update()
        width = treeview.winfo_width()
        for col in treeview["columns"]:
            treeview.column(col, anchor=tk.CENTER, width=int(width / len(treeview["columns"])) -1)

    # Associar a função de centralização ao evento de redimensionamento do frameDB2
